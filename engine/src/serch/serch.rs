...
impl Engine{
    ...
    pub fn negamax(&mut self, pos: &mut Position, serch_depth: usize) -> SearchResult{
        ...
        // To deterministicly find the best move i make the fist move outside and then enter the helper.
        let legal_moves = MoveGen::legal_moves(&mut pos);
        for m in legal_moves.iter(){
            pos.make_move(m); 
            let eval =  self.negamax_helper(pos, serch_depth-1, &evaluator, move_path, -beta, -local_alpha);
    ...
    fn negamax_helper(&self, mut pos: &mut Position, serch_depth: usize, evaluator: &Evaluator, move_path: &mut MovePath,alpha: i32, beta: i32) -> (BitMove, i32){
        ...
        // TT stuff
        let tt_pos_result = self.tt.probe(pos.zobrist_key(), serch_depth as i8 - 1 , alpha, beta);
        
        if let Some(tt_cutoff) = tt_pos_result.cutoff{
            let tt_best = tt_pos_result.best.expect("this should be a number if it was a hit");
            return (tt_best, tt_cutoff)
        }
