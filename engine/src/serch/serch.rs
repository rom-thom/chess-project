...
pub fn serch_alpha_beta_negamax(pos: &mut Position, serch_depth: usize, debug_depth: usize) -> (MovePath, i32){
    ...
    serch_alpha_beta_negamax_helper(pos, serch_depth, debug_depth, &evaluator, &mut move_path, alpha, beta)
}

fn serch_alpha_beta_negamax_helper(mut pos: &mut Position, serch_depth: usize, debug_depth: usize, evaluator: &Evaluator, move_path: &mut MovePath,alpha: i32, beta: i32) -> (MovePath, i32){
    let legal_moves = MoveGen::legal_moves(&mut pos);
    let mut best_move_path = MovePath::new_empty();
    ...
    if legal_moves.is_empty() {
        if pos.in_check(pos.current.side_to_move){
            return (move_path.clone(), -score::INF)
        }
        else{
            return (move_path.clone(), 0);
        }
    ...
    if serch_depth == 0{ // When i reach the end of a branch
        ...
        return (move_path.clone(), evaluator.evaluate(&pos));
    }
    ...
    for m in legal_moves.iter(){
        ...
        if (serch_depth - serch_depth) < (debug_depth) {//?? For debugging
            dbg!(&move_path);
        }


        let (current_move_path, temp_current_eval) = serch_alpha_beta_negamax_helper(pos, serch_depth-1, debug_depth, evaluator, move_path, -beta, -local_alpha);
...
fn test_serch(){
    ...
    dbg!(&pos);
    dbg!(serch_alpha_beta_negamax(&mut pos, 5, 0));
