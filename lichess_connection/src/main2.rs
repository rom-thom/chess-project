...
async fn play_game(
    ...
    // let mut game = engine_setup(None);
    ...
    while let Some(chunk) = gs.next().await {
        ...
        for line in text.lines().filter(|l| !l.trim().is_empty()) {
            ...
            if let Ok(evt) = serde_json::from_str::<GameEvent>(line.trim()) {
                ...
                if my_turn {
                    let real_pos = build_current_pos(&history);
                    let bit_move = alpha_beta_generator(real_pos);

                    
                    match bit_move.map(|m| m.to_string()) {
                        Some(uci) => {
...
fn build_current_pos(history:  &[&str]) -> Position{
...
fn play_last_move(history:  &[&str], game: &mut Game){
    let moves_str = match history.last() {
        Some(last) => *last,
        None => return,
    };

    if game.pos.current.side_to_move == Color::White{

    }
    dbg!(&moves_str);
    dbg!(&game.pos);
    let last_move = MoveGen::stringmove_to_bitmove(&mut game.pos, moves_str).expect("Couldn't play the move given");
...
fn engine_setup(fen_string:Option<String>) -> Game{
    //TT = 2**3
    Game::new(fen_string, 1048576)

}

fn find_best_move(game: &mut Game) -> Option<BitMove>{
    let mut tmp_pos = std::mem::replace(&mut game.pos, Position::new(None)); // This is some nasty stuf chat came with // TODO Change it
    let result = game.think(&SearchLimits{max_depth: Some(7), max_time_ms: None});
